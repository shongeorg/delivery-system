	...
	<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
			...
			{:else}
				<div class="flex flex-wrap gap-3">
					<button
						...
						class="px-4 py-2 rounded-full border-2 {selectedCategory === '' ? 'border-indigo-600 bg-indigo-50' : 'border-gray-300'} hover:border-indigo-600 transition"
					>
						All
					</button>
					{#each categories as category}
						<button
							...
							class="px-4 py-2 rounded-full border-2 {selectedCategory === category.slug ? 'border-indigo-600 bg-indigo-50' : 'border-gray-300'} hover:border-indigo-600 transition"
						>
							{category.name}
						</button>
					{/each}
		...
		{:else if getFilteredProducts().length === 0}
			<div class="text-center py-12 bg-white rounded-lg shadow">
				...
				<button onclick={clearFilter} class="mt-4 text-indigo-600 hover:text-indigo-500 font-medium">
					View all products
				</button>
			</div>
		{:else}
